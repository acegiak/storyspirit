...
// 		String ret = talker.name+": ";
// 		ArrayList<String> choices = new ArrayList<String>(Arrays.asList(general));
// 		for(Object r : villi.getRecipes(p)){
// 			MerchantRecipe recipe = (MerchantRecipe)r;
// 			if(recipe.getItemToSell().getItem() != Items.emerald){
// 				choices.add("You wan buy "+recipe.getItemToSell().getDisplayName().replace('_', ' ')+"?");
// 				choices.add("Do you need "+recipe.getItemToSell().getDisplayName().replace('_', ' ')+"?");
// 				choices.add("I have plenty of "+recipe.getItemToSell().getDisplayName().replace('_', ' ')+"!");
// 				if(recipe.getItemToBuy().getItem() == Items.emerald){
// 					points.add(new ConversationPoint("Here, Friend! Take some "+recipe.getItemToSell().getDisplayName(),20f,Math.max(1f,(float)recipe.getItemToBuy().stackSize),Math.max(1f,(float)recipe.getItemToBuy().stackSize)*-0.5f,recipe.getItemToSell()));
// 					points.add(new ConversationPoint(p.getName()+"! Here, have this "+recipe.getItemToSell().getDisplayName()+"! For you!",20f,Math.max(1f,(float)recipe.getItemToBuy().stackSize),Math.max(1f,(float)recipe.getItemToBuy().stackSize)*-0.5f,recipe.getItemToSell()));
// 					points.add(new ConversationPoint("Oh, also, I got you this "+recipe.getItemToSell().getDisplayName()+". Take it.",20f,Math.max(1f,(float)recipe.getItemToBuy().stackSize),Math.max(1f,(float)recipe.getItemToBuy().stackSize)*-0.5f,recipe.getItemToSell()));
// 					points.add(new ConversationPoint("You look like you need "+recipe.getItemToSell().getDisplayName()+". Take it.",20f,Math.max(1f,(float)recipe.getItemToBuy().stackSize),Math.max(1f,(float)recipe.getItemToBuy().stackSize)*-0.5f,recipe.getItemToSell()));
// 					points.add(new ConversationPoint("You need "+recipe.getItemToSell().getDisplayName()+" more than me.",20f,Math.max(1f,(float)recipe.getItemToBuy().stackSize),Math.max(1f,(float)recipe.getItemToBuy().stackSize)*-0.5f,recipe.getItemToSell()));
// 				}
				
// 			}
// 			if(recipe.getItemToBuy().getItem() != Items.emerald){
// 				choices.add("You got any "+recipe.getItemToBuy().getDisplayName().replace('_', ' ')+"?");
// 				choices.add("I need some "+recipe.getItemToBuy().getDisplayName().replace('_', ' ')+".");
// 				choices.add("If you see any "+recipe.getItemToBuy().getDisplayName().replace('_', ' ')+" let me know.");
// 			}
// 		}
...
// 		float rep = talker.opinionOf(p.getName());
...
// 			return ret+" Ugh.";
// 		}
// 		ConversationPoint point = points.get(Character.rand.nextInt(points.size()));
// 		talker.opinionGoes(p.getName(), point.mod);
// 		System.out.println(point.toString());
// 		if(point.give != null){
// 			if(point.give.stackSize < 1){
// 				point.give.stackSize = 1;
// 			}
// 			System.out.print("GIVING ITEM: "+point.give.toString());
// 			p.worldObj.spawnEntityInWorld(new EntityItem(p.worldObj,p.posX,p.posY,p.posZ,point.give));
// 		}
// 		villi.setCustomNameTag(talker.name);
...
// 		System.out.println(talker.name+"("+villi.getUniqueID().toString()+":"+talker.id.toString()+")'s opinion of "+p.getName()+" is "+Float.toString(talker.opinionOf(p.getName())));
